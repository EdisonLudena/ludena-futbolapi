...
from gestion.serializers.user import (
    UserSerializer,
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields   = ['is_staff', 'is_active', 'tipo_usuario'] # Añadido tipo_usuario
    ...
    def change_password(self, request):
        ...
        request.user.save()
        return Response({'message': 'Contraseña actualizada. Por favor, inicia sesión de nuevo.'})
    ...
    def stats(self, request):
        ...
        return Response({
            ...
            'coaches':   qs.filter(tipo_usuario='Coach').count(),
            'scouts':    qs.filter(tipo_usuario='Scout').count(),
            'players':   qs.filter(tipo_usuario='Player').count(),
        })